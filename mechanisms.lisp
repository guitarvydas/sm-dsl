...
(defmethod $network__Emit ((self sm-dsl-parser))
  (let ((net (stack-dsl::%top (input-network (env self)))))
	...
	(let ((mname (stack-dsl::%value (name m))))
	  (pasm:emit-string self "~&(defclass ~a (%sm) () )~%" mname)
	  (pasm:emit-string self "~&(defmethod initialize-instance :after ((self ~a))~%" mname)
	  (pasm:emit-string self "~&  (setf (%state self) :idle))~%")
	  (pasm:emit-string self "~&(defmethod react ((self ~a) msg)~%" mname)
